...
4. `build_base_headers(config)`：组装基础请求头（`Authorization/Tenant-Id/...`）；
...
- Header：在基础头上增加 `Blade-Auth: bearer <token>` 与 `Content-Type: application/json`
- Body：
...
1. `envs/<env>.env` 不存在或关键配置缺失；
2. 网络异常/超时；
3. 登录响应无法解析 JSON；
4. 登录未获取到 token（`access_token` 为空）。
